...
% stochastic simulations. This will run for multiple values of numbers of
% initially free protein to see how this changes the equilibrium saturation
% and how close it is to the maximum possible saturation.

N = 1000;    %length of ssDNA
n = 3;  %length of protein (RAD51)

MinEvents = 100;   %minimum number of events to occur (typically keep at 100)
InitialFree_Values = linspace(10,ceil(N/n),109);    %initial number of proteins that are around the lattice

k_on = 100;   %kinetic rate constant for protein binding
k_off = 10;  %kinetic rate constant for protein unbinding
w = 1;  %cooperativity constant

%memory allocation
SaturationResults = zeros(1,numel(InitialFree_Values));
TimeResults = zeros(1,numel(InitialFree_Values));
DiffFromMaxResults = zeros(1,numel(InitialFree_Values));

for i = 1:numel(InitialFree_Values)
    InitialFree = InitialFree_Values(i);    %new value for the number of free proteins
...
%     box on;
    ...
    SaturationResults(i) = Avg_Saturation;  %equilibrium saturation for each value
    TimeResults(i) = QuarterTime;   %equilibrium time for each value
    DiffFromMaxResults(i) = abs(Avg_Saturation-min(1,(n*InitialFree/N)))/min(1,(n*InitialFree/N));    %percent error between maximum possible saturation and equilibrium saturation
end

figure();
subplot(3,1,1);
scatter(InitialFree_Values,TimeResults,15,'b','filled'); hold on;
ylabel('Eq. Time'); ylim([0 max(TimeResults)]); box on;
xlim([0 max(InitialFree_Values)]);
subplot(3,1,2);
scatter(InitialFree_Values,SaturationResults,15,'r','filled'); hold on;
plot(InitialFree_Values,min(1,n.*InitialFree_Values./N),'-k');
ylabel('Eq. Saturation');   ylim([0 1]); box on;
xlim([0 max(InitialFree_Values)]);
subplot(3,1,3);
scatter(InitialFree_Values,DiffFromMaxResults,15,'g','filled');   hold on;
ylabel('% Error from Max.');    ylim([0 1]); box on;
xlabel('Total Protein Count'); xlim([0 max(InitialFree_Values)]);